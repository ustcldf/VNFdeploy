        ...
        for req_id_1, req_value_1 in request_seq_dict.iteritems():
            ...
            for req_id_2, req_value_2 in request_seq_dict.iteritems():
                if req_id_1 == req_id_2:
                    ...
                    d_mtr[req_id_1][req_id_2] = self.calc_d_val(mtr_temp_1,mtr_temp_2)
...
class VNF_adjust_pod(object):
    ...
    def pod_adjust(self, host_id, K, host_lib, requests_lib):
        ...
        #return host_lib_backup                 #there is no difference between return host_lib_backup and return host_lib
        return host_lib
...
def VNF_simulation_abs(data_back):
    fattree = fat_tree()
    fattree.K = 16
    #fattree.__init__()
    ...
    request_lib_abs = copy.deepcopy(data_back)
    ...
    host_id = req_2_host.map_req2host(route, core_link, e_link, request_lib_abs, host_lib)
    
    print "*****start pod adjust*****"
    host_lib = adjust_pod.pod_adjust(host_id, fattree.K, host_lib, request_lib_abs)
    #host_lib = adjust_pod.host_adjust(host_id, fattree.K, host_lib, request_lib_abs)
    print "after pod_adjust, the saved num: "
    pod_adjust_saved_num = adjust_pod.pod_adjust_saved(host_id,host_lib)
    print "used host ori:",host_id+1
    ...
    host_info = adjust_whole.mignate_request_all(route, core_link, e_link, request_lib_abs, host_id, host_lib)
    ...
    with open('G:/ldf/simulation/data_store/host_lib_abs.pickle', 'w') as f:
    ...
    with open('G:/ldf/simulation/data_store/saved_info_dict_abs.json', 'w') as f:
