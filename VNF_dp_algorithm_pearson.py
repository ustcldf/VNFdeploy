...
class VNF_aggregation(object):
    def calc_d_val_pearson(self, mtr1,mtr2):              #calc d value
        ...
        for i in range(row):
            ...
            pearson[i] = (col * factor_1 - factor_2 * factor_3)/(math.sqrt(col*factor_4-math.pow(factor_2,2))*math.sqrt(col*factor_5-math.pow(factor_3,2)))
        d = 0
        for i in range(row):
            d += pearson[i]
        ...
        for req_id_1, req_value_1 in request_seq_dict.iteritems():
            ...
            for req_id_2, req_value_2 in request_seq_dict.iteritems():
                if req_id_1 == req_id_2:
                    ...
                    d_mtr[req_id_1][req_id_2] = self.calc_d_val_pearson(mtr_temp_1,mtr_temp_2)
    ...
    def calc_d_val_pearson(self, mtr1,mtr2):              #calc d value
        ...
        for i in range(row):
            ...
            for j in range(col):
                ...
                factor_5 += mtr2[i][j]*mtr2[i][j]
            #print "sqrt_1:",col*factor_4-math.pow(factor_2,2)
            pearson[i] = (col * factor_1 - factor_2 * factor_3)/(math.sqrt(col*factor_4-math.pow(factor_2,2))*math.sqrt(col*factor_5-math.pow(factor_3,2))+EPSILON_F)
        d = 0
        for i in range(row):
            d += pearson[i]
...
def VNF_simulation_pearson(data_back):
    fattree = fat_tree()
    fattree.K = 16
    #fattree.__init__()
    ...
    request_lib_pearson = copy.deepcopy(data_back)
    ...
    host_id = req_2_host.map_req2host(route, core_link, e_link, request_lib_pearson, host_lib)
    print "*****start pod adjust*****"
    host_lib = adjust_pod.pod_adjust(host_id, fattree.K, host_lib, request_lib_pearson)
    #host_lib = adjust_pod.host_adjust(host_id, fattree.K, host_lib, request_lib_pearson)
    ...
    host_info = adjust_whole.mignate_request_all(route, core_link, e_link, request_lib_pearson, host_id, host_lib)
    ...
    with open('G:/ldf/simulation/data_store/host_lib_pearson.pickle', 'w') as f:
    ...
    with open('G:/ldf/simulation/data_store/saved_info_dict_pearson.json', 'w') as f:
