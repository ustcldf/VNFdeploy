...
class VNF_aggregation(object):
    def calc_d_val_cosin(self, mtr1,mtr2):              #calc d value
        #print "*****calc d value*****"
        [row,col] = mtr1.shape
        ...
        for i in range(row):
            ...
            cosin_i = factor_1/(math.sqrt(factor_2)*math.sqrt(factor_3))
            cosin += cosin_i
        ...
        for req_id_1, req_value_1 in request_seq_dict.iteritems():
            ...
            for req_id_2, req_value_2 in request_seq_dict.iteritems():
                if req_id_1 == req_id_2:
                    ...
                    d_mtr[req_id_1][req_id_2] = self.calc_d_val_cosin(mtr_temp_1,mtr_temp_2)
    ...
    def search_min_request(self, req_in_host, req_left_lib, matrix):
        ...
        for req_id, req_value in req_left_lib.iteritems():
            ...
            d = self.calc_d_val_cosin(mtr_temp,mtr_temp_1)
    ...
    def calc_d_val_cosin(self, mtr1,mtr2):              #calc d value
        ...
        for i in range(row):
            ...
            for j in range(col):
                ...
                factor_2 += mtr1[i][j]*mtr1[i][j]
                factor_3 += mtr2[i][j]*mtr2[i][j]
            cosin_i = factor_1/((math.sqrt(factor_2)*math.sqrt(factor_3))+EPSILON_F)
            cosin += cosin_i
                ...
                while(self.rich_or_poor(hp_value) == False and len(hp_c_backup) != 0):     
                    ...
                    for hr_id,hr_value in host_rich.iteritems():
                        if self.sl_in_ll(req_vnf_list, hr_value.vnf_container) and self.req_to_host(hp_value.req_container_dict[req_id], hr_value):
                            ...
                            condidate_d_dict[hr_id] = self.calc_d_val_cosin(host_matrix, req_matrix)
                        ...
                        if k in host_candinate:    #the id must in host_candinate
                            if k == i:
                                ...
                                if VNF_t_dict[k].has_key(VNF_min_dict[i]):                    #ensure that host_to_receive has the same vnf
                                    ...
                                    host_2r_d[k] = self.calc_d_val_cosin(VNF_i_mtr, h_2r_mtr)
...
def VNF_simulation_cosin(data_back):
    fattree = fat_tree()
    fattree.K = 16
    #fattree.__init__()
    ...
    request_lib_cosin = copy.deepcopy(data_back)
    ...
    host_id = req_2_host.map_req2host(route, core_link, e_link, request_lib_cosin, host_lib)
    print "*****start pod adjust*****"
    host_lib = adjust_pod.pod_adjust(host_id, fattree.K, host_lib, request_lib_cosin)
    #host_lib = adjust_pod.host_adjust(host_id, fattree.K, host_lib, request_lib_cosin)
    ...
    host_info = adjust_whole.mignate_request_all(route, core_link, e_link, request_lib_cosin, host_id, host_lib)
    ...
    #with open('G:/ldf/simulation/data_store/host_lib_cosin.pickle', 'w') as f:
    ...
    with open('G:/ldf/simulation/data_store/saved_info_dict_cosin.json', 'w') as f:
