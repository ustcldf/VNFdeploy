...
class VNF_aggregation(object):
    def weighting_list(self, list_temp):
        Q = 0.4           #total weight
        p_ratio = 0.2
        ...
        for j in range(list_len):
            if list_temp[j] <= p_ratio*max_value:
                non_peak_list.append(list_temp[j])
            else:
                peak_list.append(list_temp[j])
        non_peak_num = len(non_peak_list)
        exceed_total = 0
        for m in range(len(peak_list)):
            exceed_total += peak_list[m] - p_ratio*max_value
        
        #weighting
        for k in range(list_len):
            if list_temp[k] <= p_ratio*max_value:
                if non_peak_num != 0:
                    list_temp[k] = Q/non_peak_num*list_temp[k]
            else:
                if exceed_total != 0:
                    list_temp[k] = Q*(list_temp[k]-p_ratio*max_value)/exceed_total*list_temp[k]
        
        
        
        
    
    def calc_d_val_w_consin(self, mtr1,mtr2):              #calc d value
        #print "*****calc d value*****"
        [row,col] = mtr1.shape
        ...
        for m in range(row):
            for n in range(col):
                if normal_max_dict1[m] != 0:
                    mtr1[m][n] = mtr1[m][n]/normal_max_dict1[m]
                if normal_max_dict2[m] != 0:
                    mtr2[m][n] = mtr2[m][n]/normal_max_dict2[m]
        k = 0    
        for k in range(row):
            self.weighting_list(mtr1[k])
            self.weighting_list(mtr2[k])
        
        #cosin correlation coefficient
        w_cosin = 0
        for i in range(row):
            w_cosin_i = 0
            factor_1 = 0
            ...
            for j in range(col):
                ...
                factor_2 += mtr1[i][j]*mtr1[i][j]
                factor_3 += mtr2[i][j]*mtr2[i][j]
            if (math.sqrt(factor_2)*math.sqrt(factor_3)) == 0:
                w_cosin_i = 1
            else:
                w_cosin_i = factor_1/(math.sqrt(factor_2)*math.sqrt(factor_3))
            w_cosin += w_cosin_i
        ...
        for req_id_1, req_value_1 in request_seq_dict.iteritems():
            ...
            for req_id_2, req_value_2 in request_seq_dict.iteritems():
                if req_id_1 == req_id_2:
                    ...
                    d_mtr[req_id_1][req_id_2] = self.calc_d_val_w_consin(mtr_temp_1,mtr_temp_2)
    ...
    def search_min_request(self, req_in_host, req_left_lib, matrix):
        ...
        for req_id, req_value in req_left_lib.iteritems():
            ...
            d = self.calc_d_val_w_consin(mtr_temp,mtr_temp_1)
                ...
                while(self.rich_or_poor(hp_value) == False and len(hp_c_backup) != 0):     
                    ...
                    for hr_id,hr_value in host_rich.iteritems():
                        if self.sl_in_ll(req_vnf_list, hr_value.vnf_container) and self.req_to_host(hp_value.req_container_dict[req_id], hr_value):
                            ...
                            condidate_d_dict[hr_id] = self.calc_d_val_w_consin(host_matrix, req_matrix)
                        ...
                        if k in host_candinate:    #the id must in host_candinate
                            if k == i:
                                ...
                                if VNF_t_dict[k].has_key(VNF_min_dict[i]):                    #ensure that host_to_receive has the same vnf
                                    ...
                                    host_2r_d[k] = self.calc_d_val_w_consin(VNF_i_mtr, h_2r_mtr)
...
def VNF_simulation_w_cosin(data_back):
    fattree = fat_tree()
    fattree.K = 16
    #fattree.__init__()
    ...
    request_lib_w_cosin = copy.deepcopy(data_back)
    ...
    host_id = req_2_host.map_req2host(route, core_link, e_link, request_lib_w_cosin, host_lib)
    print "*****start pod adjust*****"
    host_lib = adjust_pod.pod_adjust(host_id, fattree.K, host_lib, request_lib_w_cosin)
    ...
    host_info = adjust_whole.mignate_request_all(route, core_link, e_link, request_lib_w_cosin, host_id, host_lib)
    ...
    with open('G:/ldf/simulation/data_store/host_lib_w_cosin.pickle', 'w') as f:
    ...
    with open('G:/ldf/simulation/data_store/saved_info_dict_w_cosin.json', 'w') as f:
